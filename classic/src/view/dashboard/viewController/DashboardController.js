Ext.define('Admin.view.dashboard.viewController.DashboardController', {
    ...
    previewCorrespondence: function (btn) {
        var record = btn.getWidgetRecord(),
            ...
            //correspodence
            if(module_id != ""){//product
                correspondence_name == "correspodence";
            }else{
                correspondence_name = "genericLetter";
            }
            
        previewCorrespondence(application_code, module_id,correspondence_name,JSON.stringify({section_id:section_id,sub_module_id:sub_module_id}));//correspodence letter
    },
    dispatchCorrespondence: function (btn) {
        var record = btn.getWidgetRecord(),
            ...
            application_code = record.get('application_code'),
            module_id = record.get('module_id');
         Ext.MessageBox.confirm('Dispatch', 'Are you sure you wish to dispatch this Correspondence to Client ?', function (btn) {
            if (btn === 'yes') {
                ...
                Ext.Ajax.request({
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        Ext.getBody().unmask();
                        toastr.error('Error Dispatching data: ' + errorThrown, 'Error Response');
                    }
                });
            } else {
   ...
   container.down('hiddenfield[name=section_id]').setValue(section_id);
   console.log(module_id);
   this.fireEvent('showSelectedQueriesApplicationMoreDetails', btn,application_code,module_id,sub_module_id,section_id,prodclass_category_id,ref_no,application_id,applicant_id,process_id);
},
