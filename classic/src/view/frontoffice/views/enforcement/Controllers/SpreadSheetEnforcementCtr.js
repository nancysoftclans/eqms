var premise_type_id="";
var sub_module="";
Ext.define('Admin.view.openOffice.enforcement.Controller.SpreadSheetEnforcementCtr', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.spreadSheetEnforcementCtr',
         ...
         setGridStore: function (obj, options) {
          ...
          loadadditionalinfo:  function(sender,record) {
              var application_id=record.data['application_id'];
              var filter = { 't1.application_id':application_id };
              var   filters = JSON.stringify(filter);
               Ext.data.StoreManager.lookup('suspectedOffenceInforstr').reload({params:{filters:filters}});
    
            },
          funcReloadspreadSheetStrs: function() {
            console.log('dd00');
                   Ext.data.StoreManager.lookup('spreadsheetenforcementpplicationcolumnsstr').reload();
                 }, 
          func_showhideSpreasheetColumn: function (chk, value) {
                  var  chk_name = chk.name;
                  var grid =this.lookupReference('enforcementgridpanel');
