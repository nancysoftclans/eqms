...
class ConfigurationsController extends Controller
{
            ...
            $this->middleware(function ($request, $next) {
                ...
                switch ($method) {
                    case 'saveConfigCommonData':
                            if ($id){
                                $action = 'Updated configuration common data';
                            } else {
                                $action = 'saved new configuration commom data';
                            }
                        break;
                    ...
                    case 'saveDocumentTypes':
                        if ($id){
                            $action = 'Updated saved document types';
                        } else {
  ...
  public function saveDocumentTypes(Request $req)
{
    try {
        $user_id = \Auth::user()->id;
        $post_data = $req->all();
        ...
        $document_extension_ids = json_decode($document_extension_ids, true); // Decode as an array
        ...
        $table_data = array(
            ...
            "property_ids" => $post_data['property_ids'],
            ...
            "is_controlled" => $post_data['is_controlled'],
            "has_restriction_id" => $post_data['has_restriction_id'],
            "owner_user_id" => $post_data['owner_user_id'],
            "owner_group_id" => $post_data['owner_group_id'],
            "notifications_id" => $post_data['notifications_id']
        ...
        $params = [];
        if (is_array($document_extension_ids) && count($document_extension_ids) > 0) {
        ...
        $res = sys_error_handler($exception->getMessage(), 2, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 1), explode('\\', __CLASS__), $user_id);
    } catch (\Throwable $throwable) {
        $res = sys_error_handler($throwable->getMessage(), 2, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 1), explode('\\', __CLASS__), $user_id);
    }
            ...
            // } 
                ...
                $qry->leftjoin('wb_trader_account as t2', 't1.importer_id', 't2.id')
                    ...
                    ->select(
                        ...
                        't4.name as region',
                        't5.name as district',
                        't2.id as importer_id',
                        't1.name as applicant_name',
                        't2.country_id as app_country_id',
                        't2.region_id as app_region_id',
                        't2.district_id as app_district_id',
                        't1.physical_address as app_physical_address',
                        't1.postal_address as app_postal_address',
                        't1.telephone_no as app_telephone',
                        't1.email as app_email'
                    )
    ...
    public function getParameterFormColumnsConfig(Request $req)
    {
        ...
        foreach ($param_columns as $key => $value) {
            ...
            //New Columns
            if ($value['label'] == ucwords('created_at')) {
                unset($param_columns[$key]);
            }
        ...
        try {
            ...
            if (validateIsnumeric($form_category_id)) {
                $qry = DB::table('par_formtype_fields as t22')
                    ...
                    't33.label', 't22.group', 't22.group_title', 't22.is_enabled', 
                    ...
                    't4.field_name as parent_combo', 't5.name as xtype',
                     ...
                     't6.has_logic', 't22.default_value', 't22.order_no', 't33.options');
    ...
    public function receiveProductsPortalApplication(Request $request)
    {
        ...
        if ($results) {
            try {
                ...
                $application_code = $results->application_code;
                $tracking_no = $results->tracking_no;
                initializeApplicationDMS($module_id, $sub_module_id, $application_code, $tracking_no . rand(10, 100), $user_id);
