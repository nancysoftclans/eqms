            ...
            try {
                if (isset($id) && $id != "") {
                    if (recordExists($table_name, $where, '')) {
                        ...
                        $success = updateRecord($table_name, $where, $table_data, 1, '');
                        //update profile pic
                        ...
                        if ($success['success'] === true) {
                            if (count($groups) > 0) {
                                ...
                                $res = deleteRecord('tra_user_group', $where, array('user_id' => $id), '');
                                $res = insertMultipleRecords('tra_user_group', $assigned_groups, 1, '');

                            }
                            //log change
                            $table_data['user_id'] = $id;
                            unset($table_data['id']);
                            insertRecord('users_logs', $table_data, 1, '');
                    ...
                    if (is_connected()) {
                        ...
                        $results = insertRecord($table_name, $table_data, 1, '');
                        if ($results['success'] == true) {
                            ...
                            if (count($groups) > 0) {
                                ...
                                insertMultipleRecords('tra_user_group', $assigned_groups, 1, '');
                            }
        ...
        try {
            DB::transaction(function () use ($user_id, &$res) {
                ...
                if ($userData) {
                    ...
                    if ($pwd_updated) {
                        insertRecord('tra_password_reset_logs', $logData, 1, '');
                        deleteRecord('tra_failed_login_attempts', $logData, ['account_id' => $user_id], '');
                        //send email
                        $vars = array(
                            '{password}' => $password,
    ...
    function changeUserPassword(Request $req)
    {
        ...
        try {
            ...
            if ($userData) {
                ...
                $pwd_updated = updateRecord('users', ['id' => $user_id], $data, $user_id, '');// User::find($user_id)->update($data);
                if (isset($pwd_updated['success']) && $pwd_updated['success']) {
                    insertRecord('tra_password_reset_logs', $logData, $user_id, '');
                    deleteRecord('tra_failed_login_attempts', ['account_id' => $user_id], $user_id, '');
                    //update dms password too
                    // $alf_res = dmsUpdateAccountPassword($encryptedEmail, $prevPwd, $newPassword);
                    // if(!isset($alf_res['success']) || !$alf_res['success']){
                    //     return $alf_res;
